...
import { ChatBubbleLeftEllipsisIcon, SparklesIcon, Bars3Icon } from '@heroicons/react/24/outline';
...
const ChatView = ({
  ...
  if (!chat) {
    return (
      <div className="flex-1 flex flex-col h-full bg-gray-50">
        {/* Header even when no chat is selected */}
        <div className="border-b border-gray-200 px-2 sm:px-4 py-3 bg-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base sm:text-lg font-semibold text-gray-900 leading-tight">DietExpert</h2>
                <p className="text-xs text-gray-500">AI Nutrition Assistant</p>
              </div>
            </div>
            <div className="flex items-center gap-1 text-xs text-gray-500">
              <SparklesIcon className="h-3 w-3" />
              <span className="hidden sm:inline">DietExpert AI</span>
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <div className="text-center max-w-sm">
            ...
            <p className="text-gray-600 text-sm leading-relaxed">
              Start a new conversation to get personalized nutrition advice, meal plans, and dietary guidance from your
              AI nutrition expert.
            </p>
          </div>
        </div>
  ...
  if (error) {
    return (
      <div className="flex-1 flex flex-col h-full bg-gray-50">
        {/* Header even when there's an error */}
        <div className="border-b border-gray-200 px-2 sm:px-4 py-3 bg-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              {/* Mobile Hamburger Menu */}
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white lg:ml-0">
      {/* Chat Header */}
      <div className="border-b border-gray-200 px-2 sm:px-4 py-3 bg-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            {/* Mobile Hamburger Menu */}
            <button onClick={onToggleSidebar} className="lg:hidden p-2 hover:bg-gray-100 rounded-md transition-colors">
              <Bars3Icon className="h-5 w-5 text-gray-600" />
            </button>
            <div>
              <h2 className="text-base sm:text-lg font-semibold text-gray-900 leading-tight">{chat.title}</h2>
